...
// //__global__ void QuanConvolutionLayer<Dtype>::Weight_Quantization_gpu(Dtype& weights)
// __global__ void Weight_Quantization_gpu(const int n_, Dtype& weights,   int bit_width_,  float range_low_,float range_high_,  QuanConvolutionParameter_RoundMethod round_method_,  QuanConvolutionParameter_RoundStrategy round_strategy_)
//   {
...
// //   Dtype* qwe;
// //   cudaMalloc(&qwe, sizeof(Dtype));    
// //   *qwe = 2.0;
// //   caffe_gpu_memcpy(sizeof(Dtype), this->blobs_[0]->gpu_data(), tmp_weight);
// // const Dtype* qwe() = const_case<const int*>(rrr)
// // const 
// //Dtype* qwe= (Dtype*)(2.0);
...
// //	caffe_gpu_powx(1, qwe , Dtype(floor(min(neg_scaling_factor, pos_scaling_factor))), scaling_factor);
...
// //	caffe_gpu_powx(1, qwe, Dtype(round(min(neg_scaling_factor, pos_scaling_factor))), scaling_factor);
...
// //	caffe_gpu_powx(1, qwe, Dtype(ceil(min(neg_scaling_factor, pos_scaling_factor))), scaling_factor);
...
//   //LOG(INFO) << "YDWU4";
//     weights = min(max((Dtype)weight_rounded, (Dtype)(min_value)), (Dtype)(max_value)) /
...
//LOG(INFO) << Q_weight[0] << "++++Q_weight" << Q_weight[1];
  ...
  //  cout << "old-range_high =" << range_high_ << ";   old-range_low =" << range_low_ << endl;
  // if(is_runtime_)
  //   {
  //     Dtype* sort_weight = tmp_weight;
  //     int qcount_ = this->blobs_[0]->count();
  //     sort(sort_weight, sort_weight+(this->blobs_[0]->count()));
